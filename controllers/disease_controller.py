...
async def get_id(id: int, disease_service: DiseaseService = Depends(get_disease_service)):
    return disease_service.get_by_id(id)

@disease_controller.post("/create", response_model=DiseaseReadDto)
async def create(disease: DiseaseWriteDto, disease_service: DiseaseService = Depends(get_disease_service)):
